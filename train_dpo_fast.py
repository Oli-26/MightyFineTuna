...
def main() -> int:
    ...
    ap.add_argument("--no-grad-ckpt", action="store_true",
                    help="disable gradient checkpointing (faster but +VRAM)")
    ...
    ap.add_argument("--pairs-file", default=None,
                    help="JSONL of pre-built pairs (one {prompt,chosen,rejected} per line). "
                         "If set, skip collect_pairs entirely.")
    args = ap.parse_args()
    ...
    if args.pairs_file:
        ...
        for line in Path(args.pairs_file).read_text().splitlines():
            ...
            r = json.loads(line)
            r.setdefault("rank_chosen", 1); r.setdefault("rank_rejected", 2); r.setdefault("model", None)
            pairs.append(r)
        total_recs = len(pairs)
        print(f"loaded {len(pairs)} pre-built pairs from {args.pairs_file}")
    else:
